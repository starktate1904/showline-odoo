...
class TestMailTemplateReset(MailCommon):
    ...
    def test_mail_template_reset_translation(self):
        ...
        def fake_load_file(translation_importer, filepath, lang, xmlids=None):
            ...
            if lang == 'fr_FR':  # fr_FR has translations
                translation_importer.model_translations['mail.template'] = {
                    'body_html': {'mail.mail_template_test': {'fr_FR': '<div>Hello Erp FR</div>'}},
        ...
        self.assertEqual(mail_template.body_html.strip(), Markup('<div>Hello Erp</div>'))
        self.assertEqual(mail_template.with_context(lang='en_GB').body_html.strip(), Markup('<div>Hello Erp</div>'))
        self.assertEqual(mail_template.with_context(lang='fr_FR').body_html.strip(), Markup('<div>Hello Erp FR</div>'))
