...
class ErpIMAP4_SSL(ErpIMAP4, IMAP4_SSL):
    pass
...
class ErpPOP3_SSL(ErpPOP3, POP3_SSL):
    pass
...
odoo_mailgate: "|/path/to/odoo-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
    ...
    def _connect__(self, allow_archived=False):  # noqa: PLW3201
        ...
        if connection_type == 'imap':
            ...
            connection = ErpIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else ErpIMAP4(server, port, timeout=MAIL_TIMEOUT)
            self._imap_login__(connection)
        elif connection_type == 'pop':
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = ErpPOP3_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else ErpPOP3(server, port, timeout=MAIL_TIMEOUT)
