...
class SocialStream(models.Model):
    ...
    def _format_facebook_message(self, message, tags):
        ...
        example:
            "Hello Erp :)" -> "Hello @[542132] Erp :)"
            "Hello Erp Social :)" -> "Hello @[542132] Erp-Social :)"
            "Hello Erp - Social :)" -> "Hello @[542132] Erp-Social :)"

        details:
            "Hello @[42] faketag Erp - Social :)" -> "Hello @ [42] faketag @[542132] Erp-Social :)""
            -> With associated tag: {name: "Erp - Social", offset: 20, length: 13}

            We take the message until the tag offset: "Hello @[42] faketag "
            and remove the fake tag in the process "Hello @ [42] faketag "
            Then we insert the forged tag "@[542132] Erp-Social"
