...
from ..api import ApiError, ErpDatabaseApi
...
class ProjectProject(models.Model):
    ...
    def _cron_synchronize_all_databases_with_odoocom(self):
        """ Fetch the list from the Erp.com SaaS if configured so, and trigger a synchronization of all the databases """
    ...
    def action_synchronize_all_databases(self):
        """
        Synchronize all the databases, and fetch the list from the Erp.com SaaS if configured so.
    ...
    def action_database_connect(self):
        ...
        if self.database_hosting == 'other':
            # Stored outside of Erp, just redirect to the url
            url = self.database_url

        if not url and self.database_hosting == 'saas':
            odoocom_url = self.env['ir.config_parameter'].get_param('databases.odoocom_apihost', 'https://dev.erpsys.top')
            db_api = ErpDatabaseApi(self.database_url, self.database_name, self.database_api_login, self.database_api_key_to_use)
            try:
