...
class ErpComXmlrpcApi(BaseXmlrpcApi):
    def list_databases(self):
        return self.execute_kw('odoo.database', 'list')


class ErpDatabaseXmlrpcApi(BaseXmlrpcApi):
...
class BaseApi:
    ...
    def post_json2(self, model, method, **kwargs):
        try:
            ...
            if self.database:
                headers['X-Erp-Database'] = self.database
...
class ErpComApi(BaseApi):
    FallbackApi = ErpComXmlrpcApi
...
class ErpDatabaseApi(BaseApi):
    FallbackApi = ErpDatabaseXmlrpcApi
