...
class TestSynchronization(TestDatabasesCommon):
    ...
    def test_synchronization_disabled(self):
        # Clearing one of the two keys is enough to disable the synchronization to Erp.com
    ...
    def test_synchronize_databases(self):
        """Synchronize the databases from the Erp.com SaaS"""
        # First-time-seen database creates a project
        Project = self.env['project.project']
        self.json2_mocked_calls['dev.erpsys.top']['odoo.database']['list'] = [
            {'name': 'odoo-sa', 'url': 'http://odoo-sa.my.odoo.test', 'login': 'some_user@domain.com', 'version': '16.0+e'},
        ]
        ...
        self.assertCountEqual(self.mock_requests_request.mock_calls[:1], [
            call('post', 'https://dev.erpsys.top/json/2/odoo.database/list', data=None, json={},
                 headers={'Authorization': 'Bearer privateKey', 'X-Erp-Database': 'openerp'}, allow_redirects=False, timeout=15),
        ])
        self.assertRecordValues(new_projects, [
            {
                ...
                'database_api_login': 'some_user@domain.com',
        ...
        self.json2_mocked_calls['dev.erpsys.top']['odoo.database']['list'] = [
            {'name': 'Erp', 'url': 'http://odoo-sa.my.odoo.test', 'login': 'other_user@domain.com', 'version': '17.0+e'},
        ]
        ...
        self.assertRecordValues(new_projects, [
            {
                ...
                'database_hosting': 'saas',
                'database_url': 'http://odoo-sa.my.odoo.test',
                'database_api_login': 'other_user@domain.com',
    ...
    def test_odoocom_db_synchronization(self):
        ...
        Project = self.env['project.project']
        self.json2_mocked_calls['dev.erpsys.top']['odoo.database']['list'] = [
            {'name': 'cron-odoo-sa', 'url': 'http://cron-odoo-sa.my.odoo.test', 'login': 'some_user@domain.com', 'version': '16.0+e'},
        ]
    ...
    def test_synchronizing_several_db_with_one_missing_its_name_and_api_info(self):
        """ In case several db gets synchronized, it shouldn't raise an Error but show it later on in the wizard
            ...
            - Create a fully legit db record by synchronizing all dbs with Erp.com
    ...
    def test_synchronization_wizard_update_from_odoocom_with_authenticate_error(self):
        ...
        wizard = self.env['databases.synchronization.wizard'].create({})
        wizard._do_update_from_odoocom()
        self.assertEqual(wizard.error_message, 'Error while listing databases from https://dev.erpsys.top: Invalid apikey\n')
    ...
    def test_synchronization_wizard_update_from_odoocom_with_call_error(self):
        ...
        wizard = self.env['databases.synchronization.wizard'].create({})
        wizard._do_update_from_odoocom()
        self.assertEqual(wizard.error_message, 'Error while listing databases from https://dev.erpsys.top: Some error\n')
    ...
    def test_user_management(self):
        ...
        self.assertCountEqual(self.mock_requests_request.mock_calls, [
            ...
            }, headers={'Authorization': 'Bearer admin_apikey', 'X-Erp-Database': 'odoo-sa'}, allow_redirects=False, timeout=15),
        ])
...
class TestXmlRpcSynchronization(TestDatabasesCommon):
    ...
    def setUp(self):
        ...
        self.call_resolution_per_uri = {
            "https://dev.erpsys.top/xmlrpc/2/common": self.mock_odoocom_xmlrpc_common,
            "https://dev.erpsys.top/xmlrpc/2/object": self.mock_odoocom_xmlrpc_object,
    ...
    def test_synchronize_databases(self):
        ...
        self.set_returned_databases_list([
            {'name': 'odoo-sa', 'login': 'some_user@domain.com', 'version': '16.0+e', 'url': 'http://odoo-sa.my.odoo.test'},
        ])
        Project.action_synchronize_all_databases()
        new_projects = Project.search([])

        self.mock_odoocom_xmlrpc_common.authenticate.assert_called_with('openerp', 'someuser@domain.com', 'privateKey', {})
        ...
        self.set_returned_databases_list([
            {'name': 'odoo-bis', 'login': 'other_user@domain.com', 'version': '17.0+e', 'url': 'http://odoo-sa.my.odoo.test'},
        ])
        action = Project.action_synchronize_all_databases()
        self.assertEqual(Project.search([]), new_projects)
        self.assertRecordValues(new_projects, [
    ...
    def test_db_synchronize_several_dbs(self):
        ...
        # Other dbs should stay untouched
        self.set_returned_databases_list([
            {'name': 'odoo', 'login': 'other_user@domain.com', 'version': '17.0+e', 'url': 'http://odoo-sa.my.odoo.test'},
        ])
        action = Project.action_synchronize_all_databases()
        wizard = self.env['databases.synchronization.wizard'].browse(action['res_id'])
        self.assertEqual(wizard.summary_message, "0 new databases, 3 updated.")
    ...
    def test_odoocom_db_synchronization(self):
        ...
        Project = self.env['project.project']
        self.set_returned_databases_list([
            {'name': 'cron-odoo-sa', 'login': 'some_user@domain.com', 'version': '16.0+e', 'url': 'http://cron-odoo-sa.my.odoo.test'},
        ])
        with freeze_time('2025-01-01 00:00'):
            Project._cron_synchronize_all_databases_with_odoocom()
        new_projects = Project.search([])
    ...
    def test_synchronization_wizard_update_from_odoocom_with_authenticate_error(self):
        ...
        wizard = self.env['databases.synchronization.wizard'].create({})
        wizard._do_update_from_odoocom()
        self.assertEqual(wizard.error_message, 'Error while listing databases from https://dev.erpsys.top: TestFaultString\n')
