...
class TestProjectProject(TransactionCase):
    ...
    def test_write_projects_without_a_url(self):
        ...
        paas_database = self.env['project.project'].create([{'name': 'Erp.sh db', 'database_hosting': 'paas', 'database_url': 'https://paas.tld'}])
    ...
    def test_action_database_connect_saas(self, mock_request):
        ...
        self.assertEqual(action, {
            'type': 'ir.actions.act_url',
            'url': 'https://dev.erpsys.top/my/databases/connect/the_database_uuid',
            'target': 'new',
        })
