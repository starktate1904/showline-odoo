...
from ..api import ApiError, ErpComApi, ErpDatabaseApi, _humanize_version
...
class DatabasesSynchronizationWizard(models.TransientModel):
    ...
    def _do_update_from_odoocom(self):
        ...
        # Get db info from Erp
        ICP = self.env['ir.config_parameter'].sudo()
        apihost = ICP.get_param('databases.odoocom_apihost', 'https://dev.erpsys.top')
        ...
        odoocom_api = ErpComApi(apihost, apidb, apiuser, apikey)
        try:
    ...
    def _do_synchronize(self):
        ...
        for db in self.database_ids:
            ...
            if version := ErpDatabaseApi.fetch_version(db.database_url):
