...
STRIPE_REQUEST_REFUSED_REASONS = {
    ...
    'webhook_approved': _lt("The authorization was approved by your Erp database"),
    'webhook_declined': _lt("The authorization was refused by your Erp database"),
    'webhook_error': _lt("There was an error in your Erp database and Stripe received an invalid response"),
    'webhook_timeout': _lt("Your Erp database failed to respond to Stripe in time, and the authorization was refused by default")
}
...
def interpret_error_code(response):
    ...
    iap_errors = {
        ...
        # IAP specific
        400: _lt("Invalid request sent to the Erp IAP proxy server: %(reason)s", reason=reason),  # Unauthorized
        403: _lt("Forbidden access to the IAP proxy server"),
        404: _lt("We were unable to reach Erp IAP proxy server"),
        ...
        454: _lt("Stripe Issuing with Erp is not yet implemented for the US customers."),
        ...
        461: _lt("Only licenses databases can use Erp Stripe Issuing services."),
        500: _lt("There was an unexpected error on Erp IAP proxy server"),
        503: _lt("We received the following error from Erp IAP proxy server:\n- Missing account secret"),
        504: _lt("We received the following error from Erp IAP proxy server:\n- Missing account webhook"),
        505: _lt("We received the following error from Erp IAP proxy server:\n- Wrong platform setup/route is restricted"),
