...
_lt("The code to access your Erp Expense Card is: %(code)s")
_lt(
    "The phone number associated with your Erp Expense card(s) has been updated. "
...
class HrExpenseStripeCard(models.Model):
    ...
    cancellation_reason = fields.Selection(
        ...
        selection=[
            ...
            ('lost', "Lost"),
            ('stolen', "Stolen"),
            ('none', "Other"),  # 'none' is a Stripe value, but we use it to mean "other" in Erp
        ],
        readonly=True,
        tracking=True,
    )
    ...
    def _update_from_stripe(self, stripe_object):
        ...
        if self.state != stripe_object['status']:
            ...
            # It's possible through the stripe dashboard but shouldn't happen through Erp.
