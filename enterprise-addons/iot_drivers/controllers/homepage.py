...
class IotBoxOwlHomePage(http.Controller):
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "Erp's IoT Box - Handlers list",
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) Erp's configuration and logger to the given logger_name to the given level.
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Erp Iot logger given an IoT handler name
