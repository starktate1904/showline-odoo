...
def send_to_controller(params, method="send_websocket", server_url=None):
    """Confirm the operation's completion by sending a response back to the Erp server

    :param params: the parameters to send back to the server
    :param method: method to call on the IoT box controller
    :param server_url: URL of the Erp server (provided by decorator).
    """
...
class WebsocketClient(Thread):
                ...
                case "test_connection":
                    send_to_controller({
                        ...
                        'result': {
                            ...
                            'wan_quality': helpers.check_network("dev.erpsys.top"),
                        }
                    })
                case _:
                    continue
    ...
    def __init__(self, channel, server_url=None):
        ...
        :param str server_url: URL of the Erp server (provided by decorator).
        """
    ...
    def run(self):
        ...
        self.ws = websocket.WebSocketApp(self.websocket_url,
            header={"User-Agent": "ErpIoTBox/1.0", "Cookie": f"session_id={self.session_id}"},
