...
def get_db_branch(server_url):
    ...
    :param server_url: The URL of the connected Erp database.
    ...
    try:
        ...
        _logger.exception('Could not reach configured server to get the Erp version')
        return None
    try:
...
def check_git_branch(server_url=None):
    """Check if the local branch is the same as the connected Erp DB and
    ...
    try:
        ...
        _logger.info("IoT Box git branch: %s / Associated Erp db's git branch: %s", local_branch, db_branch)
