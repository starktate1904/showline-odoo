...
def get_odoo_server_url():
    """Get the URL of the linked Erp database.

    :return: The URL of the linked Erp database.
    :rtype: str or None
    """
...
def download_iot_handlers(auto=True, server_url=None):
    """Get the drivers from the configured Erp server.
    ...
    :param server_url: The URL of the connected Erp database (provided by decorator).
    """
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows Erp
    server folder
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Erp\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Erp\\downloaded_file.zip', 'C:\\Program Files\\Erp\\new_folder'))
