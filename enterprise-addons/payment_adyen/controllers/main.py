...
class AdyenController(http.Controller):
    ...
    def adyen_return_from_3ds_auth(self, **data):
        ...
        The route is flagged with `save_session=False` to prevent Erp from assigning a new session
        to the user if they are redirected to this route with a POST request. Indeed, as the session
        cookie is created without a `SameSite` attribute, some browsers that don't implement the
        recommended default `SameSite=Lax` behavior will not include the cookie in the redirection
        request from the payment provider to Erp. As the redirection to the '/payment/status' page
        ...
        # of Erp in some cases. For instance, when a 3DS1 authentication is required, or for
