...
class ResCompany(models.Model):
    ...
    l10n_ke_oscu_user_agreement = fields.Boolean(
        string="Erp OSCU user agreement",
        help="Agreement is required to use Erp as an OSCU service provider.",
        tracking=True,
    )
    ...
    def write(self, vals):
        ...
        if email_is_not_sent:
            for company in email_is_not_sent.filtered(lambda c: c.l10n_ke_oscu_user_agreement and c.l10n_ke_oscu_cmc_key):
                logging_message = f"""
                    Checkbox `Erp OSCU user agreement` was set to True
                ...
                mail_body = Markup("""
                    <div style="margin: 0px; padding: 0px;">
                        <p style="margin: 0px; padding: 0px;">
                            Dear %s,
                            <br/>
                            This is a notification that you have agreed to Erp's OSCU user agreement. %s can now
                            use OSCU flows in Erp to declare your activities with the KRA.
                            <br/>
                            This is an automated e-mail and no further action is needed on your part.
                            <br/>
                            Thank you for choosing Erp for your Kenyan ERP needs.
                        </p>
                    </div>
                """) % (self.env.user.name, company.name)
                mail_subject = f" {company.name} - Erp OSCU User Agreement confirmation"
                mail_values = {
    ...
    def _l10n_ke_preinitialization_checks(self):
        ...
        if not self.l10n_ke_oscu_user_agreement:
            raise UserError(_("Please agree to the terms of use of Erp as an OSCU service provider first. "))
        error_fields = []
        on_company = False
        if not self.vat:
    ...
    def action_l10n_ke_create_branches(self):
        """ Query eTIMS for saved company branches, then create those branches in Erp. """
