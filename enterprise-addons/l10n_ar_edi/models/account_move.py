...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for inv in ar_edi_invoices:
            ...
            # in Erp for consistency, this way if an error ocurrs later in another invoice we will have the ones
            # correctly validated in ARCA in Erp (CAE, Result, xml response/request).
                ...
                if response.Errors:
                    ...
                    if '10016' in return_codes:
                        try:
                            ...
                            # verify if the invoice that is being validated in Erp has lower date than the last one registered in afip
                            ...
                            # verify if the invoice that is being validated in Erp follows the sequence of the last invoice registered in afip
    ...
    def _l10n_ar_check_afip_auth_verify_required(self):
        ...
        if still_missing:
            if len(still_missing) > 1:
                raise UserError(_(
                    'We can not post these vendor bills in Erp because the '
            ...
            raise UserError(_(
                'We can not post this vendor bill in Erp because the ARCA '
