...
class ErpFinAuth(requests.auth.AuthBase):
    """ This is used to sign the request going towards ErpFin
        e.g.:
            requests.get(ODOOFIN + '/example', auth=ErpFinAuth())
            By using `auth=ErpFinAuth(self)` when doing a http request, the request will be signed and
