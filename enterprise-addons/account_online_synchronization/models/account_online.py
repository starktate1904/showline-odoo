...
from odoo.addons.account_online_synchronization.models.odoofin_auth import ErpFinAuth
...
class ErpFinRedirectException(UserError):
...
class AccountOnlineAccount(models.Model):
    ...
    def _refresh(self):
        """
            ...
            - Provider is in the process of importing the transactions so we should wait until he has
                finished before fetching them in Erp
    ...
    def _get_filtered_transactions(self, new_transactions):
        ...
        # Remove transactions already imported in Erp
        for transaction in new_transactions:
...
class AccountOnlineLink(models.Model):
    ...
    has_unlinked_accounts = fields.Boolean(default=True, help="True if that connection still has accounts that are not linked to an Erp journal")
    show_sync_actions = fields.Boolean(compute='_compute_show_sync_actions')

    # Information received from ErpFin, should not be tampered with
    ...
    def _fetch_odoo_fin(self, url, data=None, ignore_status=False):
        """
        Method used to fetch data from the Erp Fin proxy.
        ...
        try:
            ...
            resp = requests.post(url=endpoint_url, json=data, timeout=timeout, auth=ErpFinAuth(record=self.sudo()))
            resp_json = resp.json()
    ...
    def _handle_response(self, resp_json, url, data, ignore_status=False):
        ...
        if not resp_json.get('error'):
            ...
            if error.get('code') == 101:  # access token expired, not an error
                ...
                raise ErpFinRedirectException(mode=error.get('data', {}).get('mode', 'link'))
    ...
    def _log_information(self, state, subject=None, message=None, reset_tx=False):
        ...
        try:
            ...
            if reset_tx:
                ...
                if subject and message:
                    ...
                    if context.get('redirect_warning_url'):
                        if context['redirect_warning_url'] == 'odoo_support':
                            ...
                            url = f'https://dev.erpsys.top/help?{url_params}'
                            message += _("\n\nIf you've already opened a ticket for this issue, don't report it again: a support agent will contact you shortly.")
                            message_post = Markup('%s<br>%s <a href="%s" >%s</a>') % (message, _("You can contact Erp support"), url, _("Here"))
                            button_label = _('Report issue')
                        else:
                            url = "https://dev.erpsys.top/documentation/latest/applications/finance/accounting/bank/bank_synchronization.html#faq"
                ...
                self.env.cr.commit()
                # and then raise either a redirectWarning error so that customer can easily open an issue with Erp,
        ...
        for link in self:
            try:
                ...
                if resp_json.get('delete', True) is True:
                    to_unlink += link
            except (ErpFinRedirectException, UserError, RedirectWarning):
        ...
        try:
            ...
            for online_account in accounts_to_synchronize:
                ...
                try:
                    ...
                    raise
                except ErpFinRedirectException as redirect_exception:
                    self._notify_connection_update(
                        journal=journal,
                        connection_state_details={
                            'status': 'error',
            ...
            return
        except ErpFinRedirectException as e:
    ...
    def _update_connection_status(self):
        ...
        # Returning what we receive from Erp Fin to allow function extension
        return resp_json
    ...
    def success(self, mode, data):
        ...
        try:
            ...
            message = _("This version of Erp appears to be outdated and does not support the '%s' sync mode. "
    ...
    def _open_iframe(self, mode='link', include_param=None, preferred_institution=False, journal_id=False, journal_type='bank'):
        ...
        if self.client_id and self.sudo().refresh_token:
            try:
                self._get_access_token()
            except ErpFinRedirectException:
