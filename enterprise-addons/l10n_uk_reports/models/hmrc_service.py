...
class HmrcService(models.AbstractModel):
    ...
    def _get_fraud_prevention_info(self, client_data=None):
        ...
        try:
            ...
            gov_dict['Gov-Vendor-Version'] = f'{urls.url_quote("Erp-hmrc")}={urls.url_quote(module_version)}'
            gov_dict['Gov-Client-User-IDs'] = urls.url_quote("Erp") + "=" + urls.url_quote(self.env.user.name)  # We can not percent encode the separator, same as Gov-Vendor-Version
            ...
            gov_dict['Gov-Vendor-Product-Name'] = urls.url_quote(f"Erp {version}")
            ...
            if hashed_license:
                gov_dict['Gov-Vendor-License-IDs'] = urls.url_quote("Erp") + "=" + urls.url_quote(hashed_license)
        except Exception:
