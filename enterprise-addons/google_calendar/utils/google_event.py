...
class GoogleEvent(abc.Set):
    ...
    Inspired by Erp recordset, one instance can be a single Google event or a
    ...
    def _meta_odoo_id(self, dbname):
        """Returns the Erp id stored in the Google Event metadata.
    ...
    def _load_odoo_ids_from_metadata(self, env, model):
        ...
        # Hence, we may have two Google recurrences linked to the same Erp id.
        # Therefore, we only consider Erp records without google id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Erp.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Erp (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Erp) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Erp several users might have the same email :/
