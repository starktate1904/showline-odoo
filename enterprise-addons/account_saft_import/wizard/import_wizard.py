...
class AccountSaftImportWizard(models.TransientModel):
    ...
    def _get_account_types(self):
        """ Returns a mapping between the account types accepted for the SAF-T and the types in Erp """
        # To be overriden
        return {}
    ...
    def _prepare_account_data(self, tree):
        ...
        :returns: account_mapping_ids: mapping between ids coming from the SAF-T with the ones from Erp and the balance
        """
    ...
    def _prepare_opening_balance_move(self, tree, map_accounts):
        """ Create a move if there is inconsistency between opening balance for each account and amls in Erp
    ...
    def _prepare_partner_data(self, tree):
        ...
        :returns: partner_mapping_ids: mapping between ids coming from the SAF-T with the ones from Erp
        """
        nsmap = self._get_cleaned_namespace(tree)
        partners_to_create = {}
    ...
    def _prepare_tax_data(self, tree):
        ...
        :returns: tax_mapping_ids: mapping between ids coming from the SAF-T with the ones used in Erp
        """
        nsmap = self._get_cleaned_namespace(tree)
    ...
    def action_import(self):
        ...
        # In Erp, move names follow sequences based on the year, so the checks complain
