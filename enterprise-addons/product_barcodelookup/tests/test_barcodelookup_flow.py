...
class TestBarcodelookup(HttpCase, MockAPIBarcodelookup):
    ...
    def _verify_product_data(self, product, normalized_view=False, variant_rights=True):
        self.assertEqual(product.name, "Erp Scale up")
        ...
        if not normalized_view:
            ...
            if variant_rights:
                ...
                self.assertItemsEqual(product.attribute_line_ids.value_ids.mapped('name'), ['Test gender', 'Cotton', 'Erp s.a', 'Erp', '9.75 × 8.25 in'])
            else:
