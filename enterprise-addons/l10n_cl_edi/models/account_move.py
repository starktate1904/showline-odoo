...
services reception has been received as well.
    ...
    def _l10n_cl_get_lines_vals_from_xml(self, xml_tree, vals, messages):
        ...
        for dte_line in xml_tree.findall('.//ns0:Detalle', namespaces=XML_NAMESPACES):
            ...
            # by the quantity we get the price unit we should use in Erp.
        ...
        if self.currency_id.compare_amounts(self.amount_total, xml_total_amount) != 0:
            self.message_post(
                body=Markup("<strong> %s </strong> %s") % (
                        _("Warning:"),
                        _("The total amount of the DTE\'s XML is %(xml_amount)s and the total amount calculated by Erp is %(move_amount)s. Typically this is caused by additional lines in the detail or by unidentified taxes, please check if a manual correction is needed.",
