...
class ResUsersSettings(models.Model):
    ...
    should_call_from_another_device = fields.Boolean(
        ...
        help="""Specify a phone number so that placing a call in Erp Phone will ring your preferred device (your desk phone or cell phone) and then connect you to the recipient.""",
    )
    ...
    how_to_call_on_mobile = fields.Selection(
        [("ask", "Always Ask"), ("voip", "Erp Phone"), ("phone", "Phone's Default App")],
        default="ask",
        string="Default phone app",
        help="""Choose which app to open when clicking on a phone number in the Erp Mobile app.""",
        required=True,
    )

    do_not_disturb_until_dt = fields.Datetime(
        string="Do Not Disturb until",
        help="If set, Erp Phone will be in Do Not Disturb mode until this time."
    )
