...
export function onErpChartItemClick(getters, chart) {
    return navigateInErpMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onWaterfallErpChartItemClick(getters, chart) {
    return navigateInErpMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onGeoErpChartItemClick(getters, chart) {
    return navigateInErpMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onSunburstErpChartItemClick(getters, chart) {
    return navigateInErpMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
...
export function onTreemapErpChartItemClick(getters, chart) {
    return navigateInErpMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
        const rawItem = chartJSChart.data.datasets[datasetIndex].data[index];
...
function navigateInErpMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items, chartJSChart) => {
        const env = getters.getErpEnv();
        const { datasets, labels } = chart.dataSource.getData();
...
export function onErpChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
...
export function onGeoErpChartItemHover() {
    return (event, items) => {
        if (!items.length) {
            event.native.target.style.cursor = "";
            return;
...
export async function navigateToErpMenu(menu, actionService, notificationService, newWindow) {
    if (!menu) {
