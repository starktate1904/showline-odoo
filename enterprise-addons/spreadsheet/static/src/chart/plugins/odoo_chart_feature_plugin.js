...
export class ErpChartFeaturePlugin extends ErpUIPlugin {
    ...
    getAvailableChartGranularities(chartId) {
        ...
        const matching = this.getters.getErpChartFieldMatching(chartId, filterId);
    ...
    _getChartHorizontalAxisFilter(chartId, fieldName) {
        for (const filter of this.getters.getGlobalFilters()) {
            const matching = this.getters.getErpChartFieldMatching(chartId, filter.id);
