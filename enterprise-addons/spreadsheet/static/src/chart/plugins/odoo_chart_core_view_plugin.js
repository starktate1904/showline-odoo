...
import { ErpUIPlugin } from "@spreadsheet/plugins";
import { deepEqual } from "@web/core/utils/objects";

export class ErpChartCoreViewPlugin extends ErpUIPlugin {
    static getters = /** @type {const} */ (["getChartDataSource", "getErpEnv"]);
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getErpChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getErpChartIds().includes(cmd.chartId)) {
                        continue;
                    }
                    if (this._shouldReloadDataSource(cmd.chartId, cmd.definition)) {
                        this._resetChartDataSource(cmd.chartId);
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getErpChartDataSourceId(chartId);
    ...
    _getErpChartDataSourceId(chartId) {
