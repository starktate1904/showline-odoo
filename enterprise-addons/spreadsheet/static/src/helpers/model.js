...
import { ErpSpreadsheetModel } from "@spreadsheet/model";
import { ErpDataProvider } from "@spreadsheet/data_sources/odoo_data_provider";
...
/**
 * @typedef {import("@spreadsheet").ErpSpreadsheetModel} ErpSpreadsheetModel
 */
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const odooDataProvider = new ErpDataProvider(env);
    const model = new ErpSpreadsheetModel(data, { custom: { env, odooDataProvider } }, revisions);
    return model;
}

/**
 * @param {ErpSpreadsheetModel} model
 */
export async function waitForErpSources(model) {
...
function containsLinkToErp(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeErpData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, content] of Object.entries(sheet.cells)) {
            ...
            if (containsErpFunction(content)) {
            ...
            if (containsLinkToErp(evaluatedCell.link)) {
 ...
 * @returns {boolean}
 */
function containsErpFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
