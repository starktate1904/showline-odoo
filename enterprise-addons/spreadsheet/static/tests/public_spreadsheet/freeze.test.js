...
import { freezeErpData, waitForDataLoaded } from "@spreadsheet/helpers/model";
import { ErpPivot, ErpPivotRuntimeDefinition } from "@spreadsheet/pivot/odoo_pivot";
...
test("odoo pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeErpData(model);
...
test("Empty ODOO.LIST result is frozen to an empty string", async function () {
    ...
    const frozenData = await freezeErpData(model);
