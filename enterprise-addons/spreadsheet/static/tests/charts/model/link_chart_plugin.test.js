...
test("Links between charts and ir.menus are correctly imported/exported", async function () {
    ...
    expect(exportedData.chartErpMenusReferences[chartId]).toBe(1, {
        message: "Link to odoo menu is exported",
    });
    const importedModel = new Model(exportedData, { custom: { env } });
    const chartMenu = importedModel.getters.getChartErpMenu(chartId);
...
test("Can undo-redo a LINK_ODOO_MENU_TO_CHART", async function () {
    ...
    expect(model.getters.getChartErpMenu(chartId)).toBe(undefined);
...
test("Links of Erp charts are duplicated when duplicating a sheet", async function () {
    ...
    expect(model.getters.getChartErpMenu(newChartId)).toEqual(
        model.getters.getChartErpMenu(chartId)
    );
});
