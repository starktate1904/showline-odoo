...
class MyInvoisDocument(models.Model):
    ...
    In Erp, a document represent either an invoice, or a group of PoS order.
    """
    ...
    def _myinvois_map_error(self, error):
        ...
        error_map = {
            ...
            "internal_server_error": self.env._(
                "Server error; If the problem persists, please contact the Erp support."
            ),
            # The proxy user credentials are either incorrect, or Erp does not have the permission to invoice on their behalf.
            "invalid_tin": self.env._(
                "Please make sure that your company TIN is correct, and that you gave Erp sufficient permissions on the MyInvois platform."
            ),
            ...
            'action_forbidden': self.env._('Permission to do this action has not been granted. Please ensure that Erp has sufficient permissions on the MyInvois platform.'),
        }
    ...
    def _myinvois_update_document(self, status, reason):
        """
        This method will try to update the status of a document on the platform, and if needed also the status in Erp.
    ...
    def _myinvois_set_state(self, state, message=None):
        ...
        # In Erp, the best way to represent that is by cancelling the invoice.
    ...
    def _myinvois_set_validation_fields(self, validation_result):
        ...
        # Erp expect a timezone unaware datetime in UTC, so we can safely remove the info without any more work needed.
