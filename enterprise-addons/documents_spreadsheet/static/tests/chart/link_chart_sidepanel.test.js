    ...
    serverData.models = {
        ...
        "res.users": {
            records: [
                ...
                { id: serverState.odoobotId, name: "ErpBot" },
            ],
        },
        "ir.actions": {
            records: [{ id: 1 }],
...
test("can link an odoo menu to a basic chart chart in the side panel", async function () {
    ...
    let odooMenu = model.getters.getChartErpMenu(chartId);
    ...
    odooMenu = model.getters.getChartErpMenu(chartId);
    expect(odooMenu.xmlid).toBe("documents_spreadsheet.test.menu", {
        message: "Erp menu is linked to chart",
    });
});
