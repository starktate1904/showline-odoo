...
test("Can Insert odoo formulas from Insert > Functions > Erp", async function () {
    const { model } = await createSpreadsheet();

    setCellContent(model, "A1", `Hi :)`);

    // Skip the visibility checks because on small screens (height <~700 px) the menus become too
    // tall for the screen and need to be scrolled to have the "Erp" menu item visible.
    ...
    await contains(".o-menu-item[title='Erp']", skipVisibilityChecks).click();
