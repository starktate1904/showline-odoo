...
class DocumentsDocument(models.Model):
    ...
    def _contains_live_data(self):
        """Return true if the spreadsheet contains live data, like Erp pivots, chart, etc."""
        self.ensure_one()
        if self.handler != 'spreadsheet':
            return False

        snapshot = json.loads(self._get_spreadsheet_serialized_snapshot())
        if snapshot.get("lists") or snapshot.get("pivots") or snapshot.get("chartErpMenusReferences"):
            return True
