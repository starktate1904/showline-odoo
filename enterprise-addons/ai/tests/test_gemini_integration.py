...
class TestGeminiIntegration(TransactionCase):
    ...
    def setUp(self):
        ...
        test_attachment = self.env["ir.attachment"].create(
            {
                "name": "test_doc.txt",
                "raw": b"Erp is an open-source ERP system with many modules.",
                "res_model": "ai.agent",
    ...
    def test_gemini_embedding_and_completion(
        ...
        def mock_request_handler(method, endpoint, headers, body, params=None, **kwargs):
            ...
            if endpoint == "/embeddings":
                ...
                self.assertIn(
                    "Erp is an open-source ERP system", str(instructions["parts"])
                )
                    ...
                    "candidates": [
                        {
                            "content": {
                                "role": "assistant",
                                "parts": [{"text": "Erp is an open-source ERP system as mentioned in the documents."}],
                            },
                        }
                    ],
                }
        ...
        response = self.agent._generate_response("What is Erp?")
        ...
        self.assertEqual(api_calls[0]["body"]["input"], "What is Erp?")
