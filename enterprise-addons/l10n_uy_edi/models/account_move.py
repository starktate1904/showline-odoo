...
class AccountMove(models.Model):
    ...
    def _get_l10n_latam_documents_domain(self):
        # EXTEND l10n_latam_invoice_document
        """ Only return the implemented electronic document types so far if the user want to issue from Erp """
        self.ensure_one()
    ...
    def _l10n_uy_edi_cfe_B_details(self, tax_details):
        ...
        Check the restriction on the maximum number of lines that we can report, launch a prior exception from Erp
    ...
    def _l10n_uy_edi_dummy_validation(self):
        """ When we want to skip DGI and validate only in Erp """
    ...
    def _l10n_uy_edi_complete_cfe_from_xml(self, xml_tree, l10n_uy_idreq=False):
        ...
        difference between the move total amount in Erp and the move total amount in the XML, then a message is posted
        ...
        if xml_tree.findtext(".//{*}MntPagar"):
            ...
            if not self.currency_id.is_zero(amount_total - formatted_xml_amount_total):
                ...
                errors.append(_(
                    "There is a difference between the move total amount in Erp and the move XML. Erp: %(amount_total)s  XML: %(formatted_amount_total)s.",
    ...
    def _l10n_uy_edi_get_cfe_document_type(self, xml_tree):
        """ :return: latam document type in Erp that represented the XML CFE. """
    ...
    def _l10n_uy_edi_vendor_prepare_lines(self, line_nodes, tax_included, global_discounts_surcharges=False):
        """  Prepare the lines to create vendor bills lines in Erp from the given xml.
