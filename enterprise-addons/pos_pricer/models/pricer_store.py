...
class PricerStore(models.Model):
    ...
    # The name of the Pricer store in Erp
    name = fields.Char(
        string='Store Name',
        help='Pricer Store name in Erp database',
        required=True
    )
    ...
    def unlink_label(self, pricer_tag_id):
        """
        Stop displaying product infromation on a pricer tag when deleting it from Erp database
        """
    ...
    def _update_pricer_tags(self, update_all):
        ...
        while maintaining user's pricer tags synchronized with Erp database
