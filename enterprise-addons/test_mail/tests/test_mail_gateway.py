...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_eml(self):
        ...
        mail = self.format(test_mail_data.MAIL_EML_ATTACHMENT, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>', to=f'generic@{self.alias_domain}',
                           msg_id='<cb7eaf62-58dc-2017-148c-305d0c78892f@domain.com>',
                           references='<f3b9f8f8-28fa-2543-cab2-7aa68f679ebb@domain.com>',
...
class TestMailgateway(MailGatewayCommon):
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Erpbot should be creating the message')
    ...
    def test_message_process_extra_model_res_id(self):
        ...
        # Erp 10 update: compat mode has been removed and should not work anymore
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Erp is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Erp servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Erp server -> msg_id is
...
class TestMailGatewayReplies(MailGatewayCommon):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{email.message_id} {log.message_id} {reply.message_id}',  # should contain reference to ErpExternal message, logs to fill up history
                },
                'mail_mail_values': {
    ...
    def test_routing_reply_internal_messages(self):
        """ Test routing notably between two Erps when internal messages
        ...
        RFQ-like                                        creation log
                                                        initial_msg
        Erp2 replies           creation log
                                reply                   reply
        -some internal work-                            user_notification
        Erp1 replies           reply_2                 reply_2 (incoming email)
        -some internal work-                            log
        Erp2 replies           reply_3                 reply_3 (outgoing email)

        Purpose: have references from Erp2 containing message IDs to try to
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{log.message_id} {odooext_msg.message_id} {reply.message_id}',  # should contain reference to ErpExternal message
                },
                'mail_mail_values': {
        ...
        # coming from Erp1: their reply as an incoming email
        with self.mock_mail_gateway():
            self.format_and_process(
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{log.message_id} {odooext_msg.message_id} {reply.message_id} {reply_2.message_id}',  # should contain reference to ErpExternal message
                },
                'mail_mail_values': {
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{odooext_msg.message_id} {reply.message_id} {reply_2.message_id} {reply_3.message_id}',  # should contain reference to ErpExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
