...
class TestSyncErp2MicrosoftMail(TestCommon, MailCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
    ...
    def test_change_organizer_pure_odoo_event(self):
        """
        Test that changing organizer on a pure Erp event (not synced with Microsoft)
        ...
        self.assertTrue(event.active, "Pure Erp event should not be archived when changing organizer")
