...
class TestDeleteEvents(TestCommon):
    ...
    def test_delete_simple_event_from_outlook_attendee_calendar(self):
        """
        If an attendee deletes an event from its Outlook calendar, during the sync, Erp will be notified that
        this event has been deleted BUT only with the attendees's calendar event id and not with the global one
        (called iCalUId). That means, it's not possible to match this deleted event with an Erp event.

        LIMITATION:

        Unfortunately, there is no magic solution:
            1) keep the list of calendar events ids linked to a unique iCalUId but all Erp users may not have synced
            their Erp calendar, leading to missing ids in the list => bad solution.
    ...
    def test_delete_synced_event_with_sync_config_paused(self, mock_delete):
        ...
        # Try to delete a simple event in Erp Calendar.
        self.simple_event.with_user(self.organizer_user).unlink()
        self.call_post_commit_hooks()
        self.simple_event.invalidate_recordset()
        ...
        self.assertFalse(self.simple_event.exists(), "Event must be deleted from Erp even though sync configuration is off")
