...
class TestCRMLead(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.lead_generic_email_dupes = cls.env['crm.lead'].create([
            # email based: normalized version used for email domain criterion
            {
                ...
                'name': 'Dupe1 of fp@domain.com (same email)',
                'type': 'lead',
            },
            {
                'email_from': 'FP@domain.com',
                'name': 'Dupe2 of fp@domain.com (same email)',
                'type': 'lead',
            },
            # phone_sanitized based
            {
                'email_from': 'not.fp@not.odoo.com',
                'name': 'Dupe3 of fp@domain.com (same phone sanitized)',
                'phone': '+1 202 555 0123',
                'type': 'lead',
            },
            {
                'email_from': 'not.fp@not.odoo.com',
                'phone': '+1 202 555 0123',
                'name': 'Dupe4 of fp@domain.com (same phone sanitized)',
                'type': 'lead',
            },
            # same commercial entity
            {
                'name': 'Dupe5 of fp@domain.com (same commercial entity)',
                'partner_id': cls.test_partners[1].id,
            },
            {
                'name': 'Dupe6 of fp@domain.com (same commercial entity)',
    ...
    def test_assert_initial_values(self):
        ...
        self.assertEqual(lead_generic.email_domain_criterion, 'fp@domain.com')
        self.assertEqual(lead_generic.email_normalized, 'fp@domain.com')
