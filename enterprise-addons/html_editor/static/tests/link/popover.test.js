...
describe("link preview", () => {
    ...
    test("test external link preview", async () => {
        onRpc("/html_editor/link_preview_external", () => ({
            og_description:
                "From ERP to CRM, eCommerce and CMS. Download Erp or use it in the cloud. Grow Your Business.",
            og_image: "https://dev.erpsys.top/web/image/41207129-1abe7a15/homepage-seo.png",
            og_title: "Open Source ERP and CRM | Erp",
            og_type: "website",
            og_site_name: "Erp",
            source_url: "http:/dev.erpsys.top",
        }));
        const { editor } = await setupEditor(`<p>[]<br></p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill("http:/dev.erpsys.top");
        await animationFrame();
        expect(".o_we_replace_title_btn").toHaveCount(1);
        expect(".o_extra_info_card").toHaveCount(1);
        expect(".o_we_url_link").toHaveText("Open Source ERP and CRM | Erp");
    ...
    test("test external metadata cached correctly", async () => {
        const title = "Open Source ERP and CRM | Erp";
        const description =
            "From ERP to CRM, eCommerce and CMS. Download Erp or use it in the cloud. Grow Your Business.";
        onRpc("/html_editor/link_preview_external", () => {
...
describe("label is a valid URL", () => {
    ...
    test("popover should display href URL even if label is a valid URL and differs from href", async () => {
        await setupEditor('<p><a href="https:/dev.erpsys.top">googl[]e.com</a></p>');
        await waitFor(".o-we-linkpopover", { timeout: 1500 });
        expect(queryFirst(".o-we-linkpopover a").href).toBe("https:/dev.erpsys.top");
        await click(".o_we_edit_link");
        await waitFor(".o_we_href_input_link");
        expect("input.o_we_href_input_link").toHaveValue("https:/dev.erpsys.top");
    });
