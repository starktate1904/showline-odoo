...
test("Insert pivot from odoo model", async () => {
    ...
    const insertPivotErpModel = ["insert", "insert_pivot", "insert_pivot_from_odoo_model"];
    await doMenuAction(topbarMenuRegistry, insertPivotErpModel, env);
    expect.verifySteps(["NewErpPivotSidePanel"]);
});
