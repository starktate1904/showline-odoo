...
test("link is kept when copying chart", async () => {
    ...
    expect(model.getters.getChartErpMenu(chartId).id).toBe(1);
    ...
    for (const _chartId of chartIds) {
        expect(model.getters.getChartErpMenu(_chartId).id).toBe(1);
    }
});

test("copy/paste Erp chart field matching", async () => {
    ...
    model.dispatch("PASTE", { target: [toZone("A1")] });
    const chartIds = model.getters.getChartIds(sheetId);
    expect(model.getters.getErpChartFieldMatching(chartId1, filterId).chain).toBe(
        "partner_id.company_id"
    );
    expect(model.getters.getErpChartFieldMatching(chartId2, filterId).chain).toBe(
        "user_id.company_id"
    );
    expect(model.getters.getErpChartFieldMatching(chartIds[2], filterId).chain).toBe(
        "user_id.company_id"
    );
});

test("cut/paste Erp chart field matching", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "odoo_pie" });
    insertChartInSpreadsheet(model, "odoo_bar");
    const sheetId = model.getters.getActiveSheetId();
    const [chartId1, chartId2] = model.getters.getChartIds(sheetId);
    ...
    expect(model.getters.getErpChartFieldMatching(chartIds[1], filterId).chain).toBe(
        "user_id.company_id"
    );
});
