...
test("link is kept when copying charts inside of carousel", async () => {
    ...
    expect(model.getters.getChartErpMenu("chartId1").id).toBe(1);
    expect(model.getters.getChartErpMenu("chartId2").id).toBe(1);
    ...
    expect(model.getters.getChartErpMenu(newChartId1).id).toBe(1);

    const newChartId2 = newCarousel.items[1].chartId;
    expect(model.getters.getChartErpMenu(newChartId2).id).toBe(1);
});

test("cut/paste Erp chart field matching inside carousels", async () => {
    ...
    expect(model.getters.getErpChartFieldMatching(chartIds[0], "filterId").chain).toBe(
