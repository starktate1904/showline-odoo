...
import { ErpPivot } from "@spreadsheet/pivot/odoo_pivot";
import { waitForDataLoaded } from "@spreadsheet/helpers/model";

/**
 * @typedef {import("@spreadsheet").ErpSpreadsheetModel} Model
 * @typedef {import("@spreadsheet").ErpPivotDefinition} ErpPivotDefinition
 */
...
export async function setupCollaborativeEnv(serverData) {
    ...
    const alice = new Model(model.exportData(), {
        custom: {
            env,
            odooDataProvider: new ErpDataProvider(env),
        },
...
export async function insertPivot(model, sheetId = model.getters.getActiveSheetId()) {
    ...
    if (!(ds instanceof ErpPivot)) {
        throw new Error("The pivot data source is not an ErpPivot");
    }
    await ds.load();
