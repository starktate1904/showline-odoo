...
topbarMenuRegistry.addChild("chart_data_sources", ["data"], (env) => {
    const sequence = 56;
    const numberOfCharts = env.model.getters.getErpChartIds().length;
    return env.model.getters.getErpChartIds().map((chartId, index) => ({
        id: `item_chart_${chartId}`,
        name: env.model.getters.getErpChartDisplayName(chartId),
        ...
        separator: index === env.model.getters.getErpChartIds().length - 1,
        isVisible: (env) => !env.isSmall,
    }));
});
...
const insertPivotMenu = {
    ...
    children: [
        ...
        },
        {
            ...
            name: _t("From Erp data"),
            sequence: 2,
            execute: (env) => {
                env.openSidePanel("NewErpPivotSidePanel");
            },
        },
    ],
