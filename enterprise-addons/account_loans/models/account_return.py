    ...
    def _check_suite_annual_closing(self, check_codes_to_ignore):
        ...
        if 'check_loans' not in check_codes_to_ignore:
            ...
            if not loans_exist:
                checks.append({
                    'name': _("Loans"),
                    'message': _("Erp manages your amortizations automatically. No loans were found for this period. Ensure your loans are properly registered for automatic amortizations calculation."),
                    'code': 'check_loans',
                    'result': 'todo',
                })
        return checks
