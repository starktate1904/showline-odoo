...
class HavanoEarningLine(models.Model):
    _name = 'havano.earning.line'
    _description = 'Employee Earning Line'
    _order = 'sequence, id'
    
    employee_id = fields.Many2one('hr.employee', string='Employee', required=True, ondelete='cascade')
    component_id = fields.Many2one('havano.salary.component', string='Component', domain="[('component_type', '=', 'earning')]", required=True)
            ...
            if line.component_id.code in restricted_codes:
                ...
                if existing:
                    raise ValidationError(
                        f'Component "{line.component_id.name}" can only be added once per employee!'
                    )