...
class ZimbabwePayrollController(http.Controller):
    ...
    def nssa_report_pdf(self, pay_run_id, **kwargs):
        ...
        employee_rows = []
        for payslip in payslips:
            basic_salary_values = payslip.line_ids.filtered(lambda line: line.code == "BASIC").mapped('total')
            basic = float(basic_salary_values[0] or 0.0) if basic_salary_values else 0.0
            
            pensionable = min(basic, ceiling) if ceiling > 0 else basic
            
            nssa_salary_values = payslip.line_ids.filtered(lambda line: line.code == "NSSA").mapped('total')
            employee_nssa = abs(nssa_salary_values[0]) if nssa_salary_values else 0.0
            if employee_cap > 0:
    ...
    def export_report(self, report_type, pay_run_id, format='pdf', **kwargs):
        ...
        if report_type == 'salary_breakdown':
            return self.salary_breakdown_pdf(pay_run_id)
            
        elif report_type == 'salary_summary':
            return self.salary_summary_pdf(pay_run_id)
    ...
    def _export_nssa_excel(self, pay_run):
        ...
        headers = ['Surname', 'First Name', 'Start Date', 'End Date', 'Basic Salary', 'Employee NSSA', 'Employer NSSA', 'Total NSSA']
    ...
    def _get_nec_data(self, pay_run):
        ...
        for payslip in payslips:
            ...
            overtime_values = payslip.line_ids.filtered(lambda line: line.code == "OVERTIME").mapped('total')
            overtime = float(overtime_values[0] or 0.0) if overtime_values else 0.0
            
            cola_values = payslip.line_ids.filtered(lambda line: line.code == "COLA").mapped('total')
            cola = float(cola_values[0] or 0.0) if cola_values else 0.0
            
            backpay_values = payslip.line_ids.filtered(lambda line: line.code == "BACKPAY").mapped('total')
            backpay = float(backpay_values[0] or 0.0) if backpay_values else 0.0
            
            earnings_base = basic + overtime + cola + backpay
            
            nec_values = payslip.line_ids.filtered(lambda line: line.code == "NEC").mapped('total')
            employee_nec = abs(nec_values[0]) if nec_values else 0.0
            
            nec_er_values = payslip.line_ids.filtered(lambda line: line.code == "NEC_ER").mapped('total')
            employer_nec = abs(nec_er_values[0]) if nec_er_values else 0.0
    ...
    def _get_zimdef_data(self, pay_run):
        ...
        for payslip in payslips:
            ...
            employee_rows.append({
                ...
                "total_zimdef": zimdef_employer,
            })
            
        employee_rows.sort(key=lambda x: (x['surname'] or '').lower())
        totals = {
            "basic": sum(r["basic"] for r in employee_rows),
            "zimdef_employer": sum(r["zimdef_employer"] for r in employee_rows),
    ...
    def _export_zimdef_excel(self, pay_run):
        ...
        headers = ['Surname', 'First Name', 'Basic Salary', 'Employer ZIMDEF', 'Total ZIMDEF']
        for col, header in enumerate(headers):
            sheet.write(0, col, header, bold)
            sheet.set_column(col, col, 15)
