...
class HrPayslip(models.Model):
    ...
    'hr.payslip.custom.line',
    'payslip_id',
    string="Custom Lines"
)

    manual_paye = fields.Float(string="Manual PAYE Override")
    ...
    def compute_sheet(self):
        for payslip in self:
            ...
            # 🧹 STEP 0: Clear old lines
            # 🧹 Remove ONLY system-generated lines
            payslip.line_ids.filtered(lambda l: not l.is_manual).unlink()

            # 📄 STEP 1: Get contracts
            contract_ids = payslip.contract_id.ids or \
                self.get_contract(payslip.employee_id, payslip.date_from, payslip.date_to)

            # 🧠 STEP 2: Run Odoo salary rule engine
            raw_lines = self._get_payslip_lines(contract_ids, payslip.id)

            employee = payslip.employee_id
            filtered_lines = []

            # ✅ STEP 3: Filter system-generated lines
            for line in raw_lines:

                # ❌ Skip disabled salary rules
                if line.get('salary_rule_id') in employee.disabled_rule_ids.ids:
                    continue
                ...
                if tools.float_is_zero(total, precision_digits=2):
                    continue

                line['is_manual'] = False
                filtered_lines.append((0, 0, line))

            # ➕ STEP 4A: Add employee-level salary components (PERMANENT)
            for custom in employee.salary_line_ids:
                if not custom.active or tools.float_is_zero(custom.amount, precision_digits=2):
                    continue

                category_code = 'ALW' if custom.type == 'earning' else 'DED'
                ...
                filtered_lines.append((0, 0, {
                    'name': custom.name,
                    'code': (custom.name or '')[:10].upper(),
                    'category_id': category.id if category else False,
                    'sequence': 900,  # before payslip custom
            ...
            # ➕ STEP 4B: Add payslip-level custom lines (ONE-TIME / OVERRIDES)
            for custom in payslip.custom_line_ids:
                if not custom.active or tools.float_is_zero(custom.amount, precision_digits=2):
                    continue
            ...
            # 💾 STEP 5: Write all lines
            payslip.write({
                'line_ids': filtered_lines,
                'number': number
            })

            # ✏️ STEP 6: PAYE Override (after everything exists)
            if payslip.manual_paye:
                paye_line = payslip.line_ids.filtered(
                    lambda l: l.code == 'PAYE' or (l.name and l.name.lower() == 'paye')
                )
                if paye_line:
                    paye_line.write({
                        'amount': payslip.manual_paye,
                        'quantity': 1,
                        'rate': 100
                    })
    ...
    def action_add_manual_line(self):
        for slip in self:
            slip.write({
                'line_ids': [(0, 0, {
                    'name': 'Adjustment',
